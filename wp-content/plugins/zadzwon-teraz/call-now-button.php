<?php
/*
Plugin Name: Zadzwon do Nas
Plugin URI: http://314.pl
Description: Przycisk do dzwonienia wyświetlany na stronach moblinych
Version: 0.3.1
Author: MW
Author URI: http://www.314.pl
License: GPL2
*/
...
function cta_add_inline_script() {
    $cnb_settings = cnb_get_options(); // Grabbing the settins and checking for latest version OR creating the options file for first time installations
    $cnb_options = $cnb_settings['options'];
    if (!empty($cnb_options['global_js'])) {
        wp_add_inline_script( 'cta-gtag', $cnb_options['global_js'], 'after' );
        //var_dump($cnb_settings);
        //die('test');
    }
}
//add_action( 'wp_enqueue_scripts', 'cta_add_inline_script' );
add_action( 'wp_head', function() {
    $cnb_settings = cnb_get_options(); // Grabbing the settins and checking for latest version OR creating the options file for first time installations
    $cnb_options = $cnb_settings['options'];
    if (!empty($cnb_options['global_js'])) {
        echo $cnb_options['global_js'];
        //var_dump($cnb_settings);
        //die('test');
    }
     } );
...
<form method="post" action="options.php" class="cnb-container">
    ...
    <div id="settings">
		<table class="form-table">
			...
			<tr valign="top">
				...
				<td>
				    ...
				    <div class="radio-item">
					    ...
					    <label for="tracking5">Custom 314 Google Analytics (gtag.js)</label>
				    </div>
			...
			<tr valign="top">
				<th scope="row">Tagmanager ID (send_to):</th>
		    	<td><input type="text" name="cnb[tag_sendto]" value="<?php echo $cnb_options['tag_sendto']; ?>" /></td>
		    </tr>

			<tr valign="top">
				<th scope="row">Globalny kod śledzenia (w sekcji head):</th>
		    	<td><textarea name="cnb[global_js]" style="width: 100%; min-height: 200px;" ><?php echo $cnb_options['global_js']; ?></textarea></td>
		    </tr>
...
if(get_option('cnb') && !is_admin()) {
	...
	if($enabled == '1') {
		...
		function cnb_footer() {
			...
			} elseif($alloptions['tracking'] == '4') { // for custom gtag.js
				$tracking = 'onclick="return gtag_report_conversion(\'tel:'.$alloptions['number'].'\');"';
			} else {
				$tracking = "";
			}
      ...
      if($alloptions['tracking'] == '4') {
        $callLink = '<a id="callnowbutton" '.$tracking.'>'.$buttonText.'</a>';
      } else {
