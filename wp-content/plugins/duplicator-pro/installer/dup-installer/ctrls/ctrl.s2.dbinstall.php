...
class DUPX_DBInstall
{
    ...
    public function prepareSQL()
    {
        ...
        if ($sql_file === false || strlen($sql_file) < 10) {
            ...
            $msg    = "
<b>Unable to read/find the ".esc_html($sql_file_rel_path)." file from the archive.</b><br/>
Please check these items: <br/><br/>
1. Validate permissions and/or group-owner rights on these items: <br/>
{$spacer}- File: dup-database__".esc_html($GLOBALS['DUPX_AC']->package_hash).".sql file in dup-installer folder<br/>
{$spacer}- Directory: [".esc_html($this->root_path)."] <br/>
{$spacer}<small>See: <a href='".esc_url($faq_url.'#faq-trouble-055-q')."' target='_blank'>".esc_url($faq_url.'#faq-trouble-055-q')."</a></small><br/><br/>
2. Validate the dup-database__".esc_html($GLOBALS['DUPX_AC']->package_hash).".sql file exists and is in the dup-installer folder of the archive.zip file <br/>
{$spacer}<small>See: <a href='".esc_url($faq_url.'#faq-installer-020-q')."' target='_blank'>".esc_url($faq_url.'#faq-installer-020-q')."</a></small><br/><br/> ";
            DUPX_Log::error($msg);
        }
